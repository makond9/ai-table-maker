...
const Admin = () => {
  ...
  const availableMacros = [
    { macro: '{date}', description: 'Текущая дата' },
    { macro: '{buyers_name}', description: 'Имя покупателя' },
    { macro: '{offer_name}', description: 'Название оффера' },
    { macro: '{traffic_account}', description: 'Трафиковый аккаунт' },
    { macro: '{country}', description: 'Страна' },
    { macro: '{rk}', description: 'РК' },
    { macro: '{pixel}', description: 'Пиксель' },
    { macro: '{campaign_id}', description: 'ID кампании' },
  ];
  ...
  const previewExample = campaignNameTemplate
    ...
    .replace('{buyers_name}', 'alex_v')
    .replace('{offer_name}', 'Финансы')
    .replace('{traffic_account}', 'Мета')
    .replace('{country}', 'Россия')
    .replace('{rk}', 'РК-001')
    .replace('{pixel}', 'Facebook Pixel')
    .replace('{campaign_id}', 'abc123');
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto p-6">
        ...
        <div className="max-w-4xl">
          <Card>
            <CardHeader>
              <CardTitle>Шаблон названия кампании</CardTitle>
              <CardDescription>
                Настройте формат автоматического названия кампаний с помощью макросов
              </CardDescription>
            </CardHeader>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                  {availableMacros.map(({ macro, description }) => (
                    <Badge
                      key={macro}
                      variant="secondary"
                      className="cursor-pointer hover:bg-secondary/80 justify-start p-2"
